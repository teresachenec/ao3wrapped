...
df_works = pd.DataFrame(columns=["title", "authors", "last_updated", "fandoms", "ship_types", "rating", "work_status", "ships", "characters", "additional_tags", "word_count", "kudos", "hits", "user_last_visited"])
df_works = df_works.astype({"word_count": "int32", "kudos": "int32", "hits": "int32"})
...
def parse_hist_page(soup):
	try:
		work_list = soup.find("ol", {"class": "bookmark index group"})
		for w in work_list.find_all("li", {"class": "bookmark blurb group"}):
			# Checks if work has been deleted
			if w.find("p", {"class": "message"}) != None:
				if w.find("p").text == "This has been deleted, sorry!":
					continue
			# Check if bookmark is a series
			if str(w.find("div", {"class": "header module"}).find("h4", {"class": "heading"}).find("a")).find("series") == -1:
